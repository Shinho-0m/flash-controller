...
const App = {
  ...
  init() {
    ...
    // 🔑 用 currentUser 判断登录态（最可靠）
    const currentUsername = AppData.getCurrentUserUsername();
    const users = AppData.load('users', {});
    const user = AppData.load('user');

    // 🔍 登录状态调试日志
    console.log('[init] currentUsername:', currentUsername);
    console.log('[init] users:', users);
    console.log('[init] user from storage:', user);

    // 已登录 = currentUser 有值 且 users 表里有这个账号
    this.isLoggedIn = !!(currentUsername && users && users[currentUsername]);

    console.log('[init] isLoggedIn =', this.isLoggedIn, '| reason: currentUsername=', currentUsername, 'existsInUsers=', !!(users && users[currentUsername]));
    ...
    if (this.isLoggedIn) {
      // 确保 user 数据是最新的（从 users 表恢复）
      const u = users[currentUsername];
      let userData = AppData.load('user', null);
      if (!userData || userData.username !== currentUsername) {
        // 重建 user 数据（保留游戏进度）
        userData = { ...AppData.defaultUser };
        userData.username = currentUsername;
        userData.nickname = u.nickname || currentUsername;
        AppData.save('user', userData);
      }
